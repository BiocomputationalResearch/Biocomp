...
tetraplMetalicStructure* tetraplStructCd::_tetrapl_struct_standard_cd=new tetraplMetalicStructure[2];	//To store Standard, Standard_transformed
int tetraplStructCd::_no_of_binding_atoms=4;
int tetraplStructCd::_no_of_sides=4;
int tetraplStructCd::_no_of_vertex_vertex_angles=4;
int tetraplStructCd::_no_of_features_to_compare=2;
double tetraplStructCd::_avg_devt_dist_vert_vert[]={0.0, 0.0, 0.0, 0.0};
double tetraplStructCd::_avg_devt_angle_vertex_vertex[]={0.0, 0.0, 0.0, 0.0};
double tetraplStructCd::_avg_mse[]={0.0, 0.0};
int tetraplStructCd::_larger_size=0;
vector<string> tetraplStructCd::_larger_mol_type;
vector<string> tetraplStructCd::larger_protein_name;
int tetraplStructCd::_smaller_size=0;
vector<string> tetraplStructCd::_smaller_mol_type;
vector<string> tetraplStructCd::smaller_protein_name;
int tetraplStructCd::_same_size=0;
vector<string> tetraplStructCd::_same_mol_type;
vector<string> tetraplStructCd::same_protein_name;
int tetraplStructCd::_irregular=0;
vector<string> tetraplStructCd::_irregular_mol_type;
vector<string> tetraplStructCd::irregular_protein_name;

tetraplStructCd::tetraplStructCd(metal_binding_site met_site, bool first_time_tetrapl_cd, string accn){
	_accn=accn;
	_tetrapl_site=met_site;

	assert(_no_of_binding_atoms==4 && "No. of Binding Atoms should be 4!!");
	_tetrapl_struct_comparable_cd=new tetraplMetalicStructure[2];	//Comparable and Comparable_Transformed Tetragonal_Planar_Metal Structure
	...
	if (first_time_tetrapl_cd==true){
		cout<<"\n\nForm Standard Tetragonal Plane for CD:";
		cout<<"\n\nStandard Tetragonal Plane:\n";
		cout<<"\n\nOriginal Standard Atoms:";
		_tetrapl_struct_standard_cd[0].form_standard_tetragonal_planar_CD();
		...
		_tetrapl_struct_standard_cd[0].write_atoms_pdb_format(pdb_out_standard_name);	//Generate PDB File for Standard Atoms

		_tetrapl_struct_standard_cd[1]=_tetrapl_struct_standard_cd[0].apply_transformations();
	}
}

tetraplMetalicStructure* tetraplStructCd::form_tetrapl_site(int total_tetrapl_sites){
	string metal_name=_tetrapl_site.get_metal_atom().get_atom_symbol();
	...
	_tetrapl_struct_comparable_cd[0]=tetraplMetalicStructure(_tetrapl_site.get_inner_coord_sphere(), _tetrapl_site.get_metal_atom());
	...
	_tetrapl_struct_comparable_cd[0].write_atoms_pdb_format(pdb_out_comparable_name);	//Generate PDB File for Comparable Atoms

	_tetrapl_struct_comparable_cd[1]=_tetrapl_struct_comparable_cd[0].apply_transformations();
	...
	_tetrapl_struct_standard_cd[1].write_atoms_combined_tex_format_left(_tetrapl_struct_comparable_cd[1], tex_out_name);
	...
	_tetrapl_struct_standard_cd[1].write_atoms_combined_tex_format_right(_tetrapl_struct_comparable_cd[1], tex_out_name);

	_tetrapl_struct_standard_cd[1].compare_with_standard(_tetrapl_struct_comparable_cd[1]);

	bool check_larger_accn=false, check_smaller_accn=false, check_same_accn=false, check_irregular_accn=false;
	if (_tetrapl_struct_standard_cd[1].get_structure_size()=="larger"){
		_larger_size++;
	...
	}else if (_tetrapl_struct_standard_cd[1].get_structure_size()=="smaller"){
		_smaller_size++;
	...
	}else if (_tetrapl_struct_standard_cd[1].get_structure_size()=="same"){
		_same_size++;
	...
	}else if (_tetrapl_struct_standard_cd[1].get_structure_size()=="irregular"){
		_irregular++;
	...
	_tetrapl_struct_standard_cd[0].comparison(_tetrapl_struct_comparable_cd[0], pdb_out_comparable_name);
	...
	string site_type_standard=_tetrapl_struct_standard_cd[0].assign_standard_site_type();
	_tetrapl_struct_standard_cd[1].gen_report_metal_binding_sites(_tetrapl_struct_standard_cd[0], _tetrapl_struct_comparable_cd[0], _tetrapl_struct_comparable_cd[1], metal_name, site_type_standard, _tetrapl_site.get_site_type(), fp1);
	fclose(fp1);
	...
	return _tetrapl_struct_comparable_cd;
}

void tetraplStructCd::compute_average_deviations_in_all_tetrapl_molecules(int total_tetrapl_sites){
	double* temp_side_dev=_tetrapl_struct_standard_cd[1].get_side_deviations();
	double* temp_angle_vv=_tetrapl_struct_standard_cd[1].get_angle_deviations_vertex_vertex();
	double* temp_mse=_tetrapl_struct_standard_cd[1].get_mse();
	string metal_name=_tetrapl_site.get_metal_atom().get_atom_symbol();
...
void tetraplStructCd::gen_summary_report_tetrapl_cd(int total_tetrapl_sites){
	string summary_file_tetrapl_cd="Summary_Deviation_Report_CD_tetragonal_planar.mbsi";
	FILE *fp_tetrapl_cd=fopen(summary_file_tetrapl_cd.c_str(), "w");

	if (fp_tetrapl_cd!=NULL){
		fprintf(fp_tetrapl_cd, "HEADER     PROGRAM NAME: StructureDeviation  VERSION: 1.1");
		fprintf(fp_tetrapl_cd, "\nTITLE      Summary Report for Computation of Structural Deviations of Two Tetragonal Planar CD Binding Sites with Coordination Number 3");

		fprintf(fp_tetrapl_cd, "\nREMARK     1");
		fprintf(fp_tetrapl_cd, "\nREMARK     1  No. of Tetragonal Planar Sites Compared with Standard CD ION: %d", total_tetrapl_sites);
		...
		if (larger_protein_name.size()!=0){
			...
			for (int p=0; p<larger_protein_name.size(); p++){
				fprintf(fp_tetrapl_cd, "%s ", larger_protein_name.at(p).c_str());
			}

			sort_and_count_max_sites(_larger_mol_type, "Larger", "REMARK     2", fp_tetrapl_cd);
		}else{
		...
		if (smaller_protein_name.size()!=0){
			...
			for (int p=0; p<smaller_protein_name.size(); p++){
				fprintf(fp_tetrapl_cd, "%s ", smaller_protein_name.at(p).c_str());
			}

			sort_and_count_max_sites(_smaller_mol_type, "Smaller", "REMARK     2", fp_tetrapl_cd);
		}else{
		...
		if (same_protein_name.size()!=0){
			...
			for (int p=0; p<same_protein_name.size(); p++){
				fprintf(fp_tetrapl_cd, "%s ", same_protein_name.at(p).c_str());
			}

			sort_and_count_max_sites(_same_mol_type, "Same", "REMARK     2", fp_tetrapl_cd);
		}else{
		...
		if (irregular_protein_name.size()!=0){
			...
			for (int p=0; p<irregular_protein_name.size(); p++){
				fprintf(fp_tetrapl_cd, "%s ", irregular_protein_name.at(p).c_str());
			}

			sort_and_count_max_sites(_irregular_mol_type, "Irregular", "REMARK     2", fp_tetrapl_cd);
		}else{
...
void tetraplStructCd::sort_and_count_max_sites(vector<string> mol_type, string qualifier, string remark, FILE* file_name){
