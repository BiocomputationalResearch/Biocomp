...
triplMetalicStructure* triplStructMg::_tripl_struct_standard_mg=new triplMetalicStructure[2];	//To store Standard, Standard_transformed
int triplStructMg::_no_of_binding_atoms=3;
int triplStructMg::_no_of_sides=3;
int triplStructMg::_no_of_angles_with_circum_centre=3;
int triplStructMg::_no_of_vertex_vertex_angles=3;
int triplStructMg::_no_of_features_to_compare=4;
double triplStructMg::_avg_dist_devt_from_cc=0.0;
double triplStructMg::_avg_devt_dist_vert_vert[]={0.0, 0.0, 0.0};
double triplStructMg::_avg_devt_angle_with_cc[]={0.0, 0.0, 0.0};
double triplStructMg::_avg_devt_angle_vertex_vertex[]={0.0, 0.0, 0.0};
double triplStructMg::_avg_mse[]={0.0, 0.0, 0.0, 0.0};
int triplStructMg::_nearer=0;
vector<string> triplStructMg::_nearer_mol_type;
vector<string> triplStructMg::nearer_protein_name;
int triplStructMg::_far_apart=0;
vector<string> triplStructMg::_far_mol_type;
vector<string> triplStructMg::far_apart_protein_name;
int triplStructMg::_larger_size=0;
vector<string> triplStructMg::_larger_mol_type;
vector<string> triplStructMg::larger_protein_name;
int triplStructMg::_smaller_size=0;
vector<string> triplStructMg::_smaller_mol_type;
vector<string> triplStructMg::smaller_protein_name;
int triplStructMg::_same_size=0;
vector<string> triplStructMg::_same_mol_type;
vector<string> triplStructMg::same_protein_name;

triplStructMg::triplStructMg(metal_binding_site met_site, bool first_time_tripl_mg, string accn){
	_accn=accn;
	_tripl_site=met_site;

	assert(_no_of_binding_atoms==3 && "No. of Binding Atoms should be 3!!");
	_tripl_struct_comparable_mg=new triplMetalicStructure[2];	//Comparable and Comparable_Transformed Trigonal_Planar_Metal Structure
	...
	if (first_time_tripl_mg==true){
		cout<<"\n\nForm Standard Triangle for MG:";
		cout<<"\n\nStandard Triangle:\n";
		cout<<"\n\nOriginal Standard Atoms:";
		_tripl_struct_standard_mg[0].form_standard_triangle_MG();
		...
		_tripl_struct_standard_mg[0].write_atoms_pdb_format(pdb_out_standard_name);	//Generate PDB File for Standard Atoms

		_tripl_struct_standard_mg[1]=_tripl_struct_standard_mg[0].apply_transformations();
		...
		_tripl_struct_standard_mg[1].write_atoms_pdb_format(pdb_out_standard_name);

		string tex_out_name="standard_transformed_trigonal_planar_"+metal_name;
		_tripl_struct_standard_mg[1].write_atoms_single_tex_format(tex_out_name);

		_tripl_struct_standard_mg[1].measure_direction_cosines();
	}
}

triplMetalicStructure* triplStructMg::form_tripl_site(int total_tripl_sites){
	string metal_name=_tripl_site.get_metal_atom().get_atom_symbol();
	...
	_tripl_struct_comparable_mg[0]=triplMetalicStructure(_tripl_site.get_inner_coord_sphere(), _tripl_site.get_metal_atom());
	...
	_tripl_struct_comparable_mg[0].write_atoms_pdb_format(pdb_out_comparable_name);	//Generate PDB File for Comparable Atoms

	_tripl_struct_comparable_mg[1]=_tripl_struct_comparable_mg[0].apply_transformations();
	...
	_tripl_struct_comparable_mg[1].write_atoms_pdb_format(pdb_out_comparable_name);
	...
	_tripl_struct_comparable_mg[1].write_atoms_single_tex_format(tex_out_name);
	...
	_tripl_struct_standard_mg[1].write_atoms_combined_tex_format(_tripl_struct_comparable_mg[1], tex_out_name);

	_tripl_struct_standard_mg[1].compare(_tripl_struct_comparable_mg[1]);

	bool check_nearer_accn=false, check_far_accn=false, check_larger_accn=false, check_smaller_accn=false, check_same_accn=false;
	if (_tripl_struct_standard_mg[1].get_nearer()==true){
		_nearer++;
	...
	if (_tripl_struct_standard_mg[1].get_structure_size()=="larger"){
		_larger_size++;
	...
	}else if (_tripl_struct_standard_mg[1].get_structure_size()=="smaller"){
		_smaller_size++;
	...
	}else if (_tripl_struct_standard_mg[1].get_structure_size()=="same"){
		_same_size++;
	...
	string site_type_standard=_tripl_struct_standard_mg[0].assign_standard_site_type();
	_tripl_struct_standard_mg[1].gen_report_metal_binding_sites(_tripl_struct_standard_mg[0], _tripl_struct_comparable_mg[0], _tripl_struct_comparable_mg[1], metal_name, site_type_standard, _tripl_site.get_site_type(), fp1);
	fclose(fp1);
	...
	_tripl_struct_standard_mg[0].comparison(_tripl_struct_comparable_mg[0], pdb_out_comparable_name);

	return _tripl_struct_comparable_mg;
}

void triplStructMg::compute_average_deviations_percentages_in_all_tripl_molecules(int total_tripl_sites){
	double* temp_side_dev=_tripl_struct_standard_mg[1].get_side_deviations();
	double* temp_angle_cc=_tripl_struct_standard_mg[1].get_angle_deviations_with_cc();
	double* temp_angle_vv=_tripl_struct_standard_mg[1].get_angle_deviations_vertex_vertex();
	double* temp_mse=_tripl_struct_standard_mg[1].get_mse();
	string metal_name=_tripl_site.get_metal_atom().get_atom_symbol();

	if (total_tripl_sites==1){
		_avg_dist_devt_from_cc=_tripl_struct_standard_mg[1].get_distance_deviations_from_cc();
		...
		_avg_dist_devt_from_cc=(_avg_dist_devt_from_cc+_tripl_struct_standard_mg[1].get_distance_deviations_from_cc())/2.0;
...
void triplStructMg::gen_summary_report_tripl_mg(int total_tripl_sites){
	string summary_file_tripl_na="Summary_Deviation_Report_MG_trigonal_planar.mbsi";
	FILE *fp_tripl_mg=fopen(summary_file_tripl_na.c_str(), "w");

	fprintf(fp_tripl_mg, "HEADER     PROGRAM NAME: StructureDeviation VERSION: 1.1");
	fprintf(fp_tripl_mg, "\nTITLE      Summary Report for Computation of Structural Deviations of Two Trigonal Planar MG Binding Sites with Coordination Number 3");

	fprintf(fp_tripl_mg, "\nREMARK     1");
	fprintf(fp_tripl_mg, "\nREMARK     1  No. of Trigonal Planar Sites Compared with Standard MG ION: %d", total_tripl_sites);
	...
	if (nearer_protein_name.size()!=0){
		...
		for (int p=0; p<nearer_protein_name.size(); p++){
			fprintf(fp_tripl_mg, "%s ", nearer_protein_name.at(p).c_str());
		}

		sort_and_count_max_sites(_nearer_mol_type, "Nearer", "REMARK     2", fp_tripl_mg);
	}else{
	...
	if (far_apart_protein_name.size()!=0){
		...
		for (int p=0; p<far_apart_protein_name.size(); p++){
			fprintf(fp_tripl_mg, "%s ", far_apart_protein_name.at(p).c_str());
		}

		sort_and_count_max_sites(_far_mol_type, "Far_Apart", "REMARK     2", fp_tripl_mg);
	}else{
	...
	if (larger_protein_name.size()!=0){
		...
		for (int p=0; p<larger_protein_name.size(); p++){
			fprintf(fp_tripl_mg, "%s ", larger_protein_name.at(p).c_str());
		}

		sort_and_count_max_sites(_larger_mol_type, "Larger", "REMARK     2", fp_tripl_mg);
	}else{
	...
	if (smaller_protein_name.size()!=0){
		...
		for (int p=0; p<smaller_protein_name.size(); p++){
			fprintf(fp_tripl_mg, "%s ", smaller_protein_name.at(p).c_str());
		}

		sort_and_count_max_sites(_smaller_mol_type, "Smaller", "REMARK     2", fp_tripl_mg);
	}else{
	...
	if (same_protein_name.size()!=0){
		...
		for (int p=0; p<same_protein_name.size(); p++){
			fprintf(fp_tripl_mg, "%s ", same_protein_name.at(p).c_str());
		}

		sort_and_count_max_sites(_same_mol_type, "Same", "REMARK     2", fp_tripl_mg);
	}else{
...
void triplStructMg::sort_and_count_max_sites(vector<string> mol_type, string qualifier, string remark, FILE* file_name){
