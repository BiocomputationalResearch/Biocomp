#ifndef TETRAGOCDLPYR_STRUCTURE_CD_H_
#define TETRAGOCDLPYR_STRUCTURE_CD_H_

#include <assert.h>
#include "coordinationsite.h"
//#include "trigocdl_bipyramidal.h"
#include "tetragonal_pyramidal.h"
#include "stdio.h"

class tetrapyrStructCd{
	string _accn;
	...
	static tetrapyrMetalicStructure* _tetrapyr_struct_standard_cd;
	tetrapyrMetalicStructure* _tetrapyr_struct_comparable_cd;
	string _site_type_comparable;
	static double _avg_dist_devt_from_cc;
	static double _avg_devt_dist_vert_vert[8];	//Average deviation of edges of two tetragocdl pyramidal sites
...
public:
	tetrapyrStructCd(){}
	tetrapyrStructCd(metal_binding_site, bool, string);
	tetrapyrMetalicStructure* form_tetrapyr_site(int);
	static void gen_summary_report_tetrapyr_cd(int);
};

#endif /* TETRAGOCDLPYR_STRUCTURE_CD_H_ */
