#ifndef TETRAGOMGLPYR_STRUCTURE_MG_H_
#define TETRAGOMGLPYR_STRUCTURE_MG_H_

#include <assert.h>
#include "coordinationsite.h"
//#include "trigomgl_bipyramidal.h"
#include "tetragonal_pyramidal.h"
#include "stdio.h"

class tetrapyrStructMg{
	string _accn;
	...
	static tetrapyrMetalicStructure* _tetrapyr_struct_standard_mg;
	tetrapyrMetalicStructure* _tetrapyr_struct_comparable_mg;
	string _site_type_comparable;
	static double _avg_dist_devt_from_cc;
	static double _avg_devt_dist_vert_vert[8];	//Average deviation of edges of two tetragomgl pyramidal sites
...
public:
	tetrapyrStructMg(){}
	tetrapyrStructMg(metal_binding_site, bool, string);
	tetrapyrMetalicStructure* form_tetrapyr_site(int);
	static void gen_summary_report_tetrapyr_mg(int);
};

#endif /* TETRAGOMGLPYR_STRUCTURE_MG_H_ */
